...
let undoStack = [];
const UNDO_LIMIT = 15;; // { row, index } - for Ctrl+Z undo, cleared after use
...
async function fetchSummary() {
  ...
  if (focusProduct) {
    // Under product focus, KPIs describe just that product, computed
    // client-side from the (already-focused) rows.
    const rows = allRows.filter((r) => r.productName === focusProduct);
    ...
    document.getElementById('kpiProducts').textContent = rows.length ? 1 : 0;
...
function render() {
  ...
  emptyState.textContent = focusProduct && allRows.length > 0
    ? `No warehouse rows found for "${focusProduct}".`
    : 'No data yet. Upload a report to get started.';
...
tableBody.addEventListener('click', (e) => {
  ...
  const row = allRows.find((r) => rowKey(r) === key) || (lastDeleted && rowKey(lastDeleted.row) === key ? lastDeleted.row : null);
...
async function undoLastDelete() {
  ...
  const item = undoStack[undoStack.length - 1]; // peek, not pop
  ...
  try {
    ...
    undoStack.pop(); // only remove on confirmed success
    ...
    // leave it in undoStack so the user can just press Ctrl+Z again
    uploadStatus.textContent = 'Undo failed, try again: ' + err.message;
    uploadStatus.className = 'upload-status error';
  }
}
