...
let undoStack = [];
const UNDO_LIMIT = 15; // { periodKey, storeIndex, campaign } - for Ctrl+Z undo
...
function render() {
  ...
  // Campaigns aren't explicitly tagged with a product - the best signal
  // we have is whether the campaign's own name/detail mentions it. If
  // that leaves nothing, fall back to showing everything rather than an
  // empty table, since the absence of a match doesn't necessarily mean
...
async function handleRowDelete(row, btn) {
  ...
  try {
    ...
    lastDeleted = { periodKey: selectedPeriodKey, storeIndex: row.storeIndex, campaign: result.removed || row };
...
tableBody.addEventListener('click', (e) => {
  ...
  const row =
    ...
    (lastDeleted && lastDeleted.storeIndex === storeIndex ? { ...lastDeleted.campaign, storeIndex } : null);
...
async function undoLastDelete() {
  if (!lastDeleted || lastDeleted.periodKey !== selectedPeriodKey) return;
  const { periodKey, storeIndex, campaign } = lastDeleted;
  lastDeleted = null;
